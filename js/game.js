...
/* The Seer learns exactly what someone is, not merely whether they're the one. */
function applyLook(state, targetId) {
  const p = byId(state, targetId);
  const role = p ? p.role : '';
  state.seerAnswer = { targetId: targetId, role: role, isKiller: role === 'killer' };
